...
import {
  addNewsetting,
  fetchSettings,
} from "@/app/action/setting/dbOperations";
import { Button } from "@/components/ui/button";

const Page = () => {
  const [settingName, setSettingName] = useState<settingSchemaType[]>([]);
  ...
  async function onsubmit(data: settingSchemaType) {
    ...
    formData.append("settingName", data.settingName.toLowerCase());
                  ...
                  <div className="flex flex-col gap-1 w-full">
                    ...
                    <span className="text-[0.8rem] font-medium text-destructive">
                      {errors.settingName?.message && (
                        <p>{errors.settingName?.message}</p>
                      )}
                    </span>
                  </div>
